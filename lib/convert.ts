export async function convertToArabic(latinText: string, toFusha: boolean): Promise<string> {
  try {
    const response = await fetch("/api/convert", {
      ...
      body: JSON.stringify({ latinText, toFusha }),
    })
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      throw new Error(`Received non-JSON response. Status: ${response.status} ${response.statusText}`)
    }
  } catch (error) {
